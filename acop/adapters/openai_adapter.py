"""OpenAI provider adapter — closed-catalog mode, second-provider arm.

Mirrors `anthropic_adapter.py` deliberately: same system prompt, same user
message builder, same `parse_choice()` on raw free-text output. No provider-
specific structured-output mode (e.g. `response_format={"type":"json_object"}`)
is used, even though OpenAI supports it — giving one provider guaranteed-valid
JSON and the other free-form prose to survive would confound a per-provider
rank-correlation comparison with a parsing-difficulty difference instead of a
genuine model-behavior difference. Same failure modes, same parser, same rules.

Real-probe code — every call costs money:
...
both against current OpenAI docs before any large run. Reasoning-class models
(o-series, gpt-5-thinking, ...) may reject an explicit `temperature` other
than their default and use different token-budget semantics than chat models —
check the specific PROVIDER_B_MODEL against current docs before dispatch.
"""
...
ADAPTER_VERSION = "openai_closed_catalog@1.0.0"
...
class OpenAIBuyerAdapter:
    ...
    def __init__(
        ...
        import openai  # lazy: repo runs without the SDK installed
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                resp = self._client.chat.completions.create(
                    model=self.model,
                    max_completion_tokens=self.max_tokens if not diagnostic else self.max_tokens + 300,
